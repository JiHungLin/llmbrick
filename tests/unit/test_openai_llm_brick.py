...
import pytest
from openai.types.chat import ChatCompletion, ChatCompletionChunk, Choice, ChatCompletionMessage
    ...
    chunks = [
        ChatCompletionChunk(
            ...
            choices=[
                Choice(
                    index=0,
                    delta=ChatCompletionMessage(role="assistant", content=text),
                    finish_reason=None
                )
            ]
        )
...
async def test_get_service_info(mock_api_key):
    ...
    assert "gpt-3.5-turbo" in [m.model_id for m in info.models]
    assert "gpt-4" in [m.model_id for m in info.models]
    assert "gpt-4o" in [m.model_id for m in info.models]
...
async def test_api_error_handling(mock_api_key, mock_openai_client):
    ...
    assert "API Error" in response.error.message
    ...
    assert "API Error" in responses[0].error.message